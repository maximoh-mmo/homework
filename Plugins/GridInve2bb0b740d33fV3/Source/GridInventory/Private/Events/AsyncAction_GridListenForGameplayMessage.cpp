...
#include "Events/AsyncAction_GridListenForGameplayMessage.h"
...
UAsyncAction_GridListenForGameplayMessage* UAsyncAction_GridListenForGameplayMessage::ListenForGridGameplayMessages(UObject* WorldContextObject, FGameplayTag Channel, UScriptStruct* PayloadType, EGridGameplayMessageMatch MatchType)
{
	...
	UAsyncAction_GridListenForGameplayMessage* Action = NewObject<UAsyncAction_GridListenForGameplayMessage>();
...
void UAsyncAction_GridListenForGameplayMessage::Activate()
{
	if (UWorld* World = WorldPtr.Get())
	{
		if (UGridGameplayMessageSubsystem::HasInstance(World))
		{
			UGridGameplayMessageSubsystem& Router = UGridGameplayMessageSubsystem::Get(World);

			TWeakObjectPtr<UAsyncAction_GridListenForGameplayMessage> WeakThis(this);
			ListenerHandle = Router.RegisterListenerInternal(ChannelToRegister,
				...
				{
					if (UAsyncAction_GridListenForGameplayMessage* StrongThis = WeakThis.Get())
					{
...
void UAsyncAction_GridListenForGameplayMessage::SetReadyToDestroy()
{
...
bool UAsyncAction_GridListenForGameplayMessage::GetPayload(int32& OutPayload)
{
	checkNoEntry();
	return false;
}

DEFINE_FUNCTION(UAsyncAction_GridListenForGameplayMessage::execGetPayload)
{
...
void UAsyncAction_GridListenForGameplayMessage::HandleMessageReceived(FGameplayTag Channel, const UScriptStruct* StructType, const void* Payload)
{
