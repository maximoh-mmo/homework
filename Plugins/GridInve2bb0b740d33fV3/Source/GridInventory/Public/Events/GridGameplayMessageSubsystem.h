...
#include "GridGameplayMessageSubsystem.generated.h"

class UGridGameplayMessageSubsystem;
struct FFrame;

GRIDINVENTORY_API DECLARE_LOG_CATEGORY_EXTERN(LogGridGameplayMessageSubsystem, Log, All);

class UAsyncAction_GridListenForGameplayMessage;
...
struct GRIDINVENTORY_API FGridGameplayMessageListenerHandle
{
public:
	GENERATED_BODY()

	FGridGameplayMessageListenerHandle() {}
...
private:
	UPROPERTY(Transient)
	TWeakObjectPtr<UGridGameplayMessageSubsystem> Subsystem;
	...
	friend UGridGameplayMessageSubsystem;

	FGridGameplayMessageListenerHandle(UGridGameplayMessageSubsystem* InSubsystem, FGameplayTag InChannel, int32 InID) : Subsystem(InSubsystem), Channel(InChannel), ID(InID) {}
};
 ...
 */
USTRUCT()
struct FGridGameplayMessageListenerData
{
	GENERATED_BODY()
	...
	int32 HandleID;
	EGridGameplayMessageMatch MatchType;
 ...
 */
UCLASS()
class GRIDINVENTORY_API UGridGameplayMessageSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

	friend UAsyncAction_GridListenForGameplayMessage;
...
public:
	 ...
	 */
	static UGridGameplayMessageSubsystem& Get(const UObject* WorldContextObject);
	 ...
	 */
	template <typename FMessageStructType>
	FGridGameplayMessageListenerHandle RegisterListener(FGameplayTag Channel, TFunction<void(FGameplayTag, const FMessageStructType&)>&& Callback, EGridGameplayMessageMatch MatchType = EGridGameplayMessageMatch::ExactMatch)
	{
	...
	FGridGameplayMessageListenerHandle RegisterListener(FGameplayTag Channel, TOwner* Object, void(TOwner::* Function)(FGameplayTag, const FMessageStructType&))
	{
	 ...
	 */
	template <typename FMessageStructType>
	FGridGameplayMessageListenerHandle RegisterListener(FGameplayTag Channel, FGridGameplayMessageListenerParams<FMessageStructType>& Params)
	{
		FGridGameplayMessageListenerHandle Handle;
	 ...
	 */
	void UnregisterListener(FGridGameplayMessageListenerHandle Handle);
...
private:
	...
	FGridGameplayMessageListenerHandle RegisterListenerInternal(
		FGameplayTag Channel, 
...
private:
	...
	{
		TArray<FGridGameplayMessageListenerData> Listeners;
		int32 HandleID = 0;
	};
